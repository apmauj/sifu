...
class PerformanceBudgetManager:
    ...
    def update_throughput(self, endpoint: str = "global"):
        ...
        with self._lock:
            ...
            if window_seconds > 0:
                metrics.requests_per_minute = metrics.request_count / (
                    window_seconds / 60
                )
    ...
    def get_budget_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for name, budget in self._budgets.items():
                ...
                current_value = self._get_current_budget_value(
                    budget, global_stats, global_throughput
                )
                status[name] = {
                    "budget": budget.name,
                    "type": budget.budget_type.value,
    ...
    def _calculate_budget_status(
        self, budget: PerformanceBudget, current_value: float
    ) -> str:
...
def get_performance_budget_manager(enable_monitoring=False, enable_alerts=False):
    ...
    if _performance_budget_manager_instance is None:
        _performance_budget_manager_instance = PerformanceBudgetManager(
            enable_monitoring=enable_monitoring, enable_alerts=enable_alerts
        )
