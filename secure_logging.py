...
class SecureLogFormatter(logging.Formatter):
    ...
    def __init__(
        self, encrypt_sensitive: bool = False, encryption_key: Optional[str] = None
    ):
        super().__init__()
        ...
        if self.encrypt_sensitive and encryption_key:
            ...
            print(
                "Warning: cryptography module not available, disabling log encryption"
            )
...
class SecurityLogger:
    ...
    def log_authentication(
        self, success: bool, username: str, ip_address: str, user_agent: str = "N/A"
    ):
        """Log authentication attempt"""
        event_type = "SUCCESS" if success else "FAILURE"
        message = f"Authentication {event_type} for user: {username}"
        extra_data = {"user_agent": user_agent, "auth_success": success}
    ...
    def log_suspicious_activity(
        self, ip_address: str, activity: str, details: str = ""
    ):
    ...
    def log_config_change(
        self, change_type: str, key: str, old_value: str = "N/A", new_value: str = "N/A"
    ):
        """Log configuration changes"""
        message = (
            f"Configuration change: {change_type} - {key}: {old_value} -> {new_value}"
        )
        # Encrypt sensitive config changes
        if self.log_encryptor and any(
            sensitive in key.upper() for sensitive in ["SECRET", "KEY", "PASSWORD"]
        ):
            if old_value != "N/A":
    ...
    def log_data_access(
        ...
        self.log_security_event(
            "DATA_ACCESS", message, user_id=user_id, ip_address=ip_address
        )
        ...
        try:
            with open(self.log_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if ip_match:
                        ip = ip_match.group(1)
                        analysis["ip_addresses"][ip] = (
                            analysis["ip_addresses"].get(ip, 0) + 1
                        )
