...
class ConfigurationValidator:
    ...
    def _validate_required_secrets(self):
        ...
        if missing_secrets:
            self.errors.append(f"Missing required secrets: {', '.join(missing_secrets)}")
            self.errors.append("Please set these environment variables or use the .env.template file")
    ...
    def _validate_database_config(self):
        ...
        if not db_url.startswith(('postgresql://', 'sqlite:///', 'mysql://')):
            self.errors.append("DATABASE_URL must start with postgresql://, sqlite:///, or mysql://")

        # Check for insecure configurations
        if 'password' in db_url and 'localhost' not in db_url:
            self.warnings.append("Database URL contains password - ensure connection is encrypted in production")

        # SQLite specific validations
        if db_url.startswith('sqlite:///'):
            db_path = db_url.replace('sqlite:///', '')
            if db_path != ':memory:' and not Path(db_path).parent.exists():
                self.warnings.append(f"SQLite database directory does not exist: {Path(db_path).parent}")
    ...
    def _validate_security_config(self):
        ...
        if api_key and len(api_key) < 20:
            self.warnings.append("API_KEY should be at least 20 characters long for security")
        ...
        if environment not in ['development', 'staging', 'production']:
            self.warnings.append(f"ENVIRONMENT should be one of: development, staging, production. Got: {environment}")
        ...
        if log_level not in valid_levels:
            self.warnings.append(f"LOG_LEVEL should be one of: {', '.join(valid_levels)}. Got: {log_level}")
    ...
    def _validate_cors_config(self):
        ...
        if not allow_origins:
            if self._secrets.get('ENVIRONMENT') == 'production':
                ...
                self.warnings.append("ALLOW_ORIGINS not configured - using restrictive defaults")
            return
        ...
        if '*' in allow_origins and env_check == 'production':
            self.errors.append("Wildcard (*) in ALLOW_ORIGINS is not allowed in production")

        # Validate origin formats
        origins = [o.strip() for o in allow_origins.split(',') if o.strip()]
        for origin in origins:
            if not origin.startswith(('http://', 'https://')) and origin != '*':
                self.errors.append(f"Invalid CORS origin format: {origin}. Must start with http:// or https://")
        ...
        if environment == 'production':
            ...
            for var in required_prod_vars:
                if var not in self._secrets or not self._secrets[var]:
                    self.errors.append(f"{var} must be configured in production environment")
            ...
            if Path(file_path).exists():
                ...
                try:
                    ...
                    if stat_info.st_mode & 0o004:
                        self.warnings.append(f"File {file_path} is world-readable. Consider restricting permissions.")
                except OSError:
