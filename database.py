...
from constants import (
    DATABASE_URL, DATABASE_CONNECT_ARGS, TABLE_UI_RECORDS, TABLE_UR_RECORDS, TABLE_EXCHANGE_RATE_RECORDS, TABLE_BROU_RECORDS
)
...
class ExchangeRateRecord(Base):
    ...
    currency = Column(String(10), nullable=False, index=True)  # Currency code (USD, EUR, etc.)
...
class BROURecord(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    currency = Column(String(20), nullable=False, index=True)  # USD, EUR, ARS, BRL, USD_EBROU
