...
class TestAPISimple:
    ...
    def test_get_ui_range_success(self, client):
        ...
        data = response.json()
        assert data["success"] is True
        assert "UI values for range 2024-01-01 - 2024-01-02 retrieved successfully" in data["message"]
    ...
    def test_get_info_success(self, client):
        ...
        with patch('main.UIService') as mock_service_class:
            ...
            mock_service.get_date_range_available.return_value = (date(2002, 6, 1), date(2024, 12, 1))
