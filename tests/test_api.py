...
class TestUIEndpoints:
    ...
    def test_get_ui_by_date_closest_found(self, client, mock_ui_service):
        ...
        data = response.json()
        assert data["success"] is True
        assert "No data for 2024-01-15. Showing closest previous value" in data["message"]
    ...
    def test_get_ui_by_date_invalid_format(self, client, mock_ui_service):
        ...
        assert response.status_code == 422  # FastAPI devuelve 422 para validación de parámetros
    ...
    def test_get_ui_range_success(self, client, mock_ui_service):
        ...
        data = response.json()
        assert data["success"] is True
        assert "UI values for range 2024-01-01 - 2024-01-02 retrieved successfully. 2 records found" in data["message"]
    ...
    def test_get_ui_range_not_found(self, client, mock_ui_service):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True  # La API siempre devuelve success=True, incluso con lista vacía
        assert "UI values for range 2024-01-01 - 2024-01-02 retrieved successfully. 0 records found" in data["message"]
        assert data["data"] == []
    ...
    def test_refresh_ui_data_success(self, client):
        ...
        with patch('main.excel_processor') as mock_processor:
            mock_processor.refresh_data.return_value = (True, "Data updated successfully", 8436)
            
            with patch('main.UIService') as mock_service_class:
                mock_service = mock_service_class.return_value
                mock_service.get_total_records.return_value = 8436
                mock_service.get_latest_ui.return_value = UIValue(date=date(2024, 12, 1), value=6.0)
    ...
    def test_refresh_ui_data_exception(self, client):
        ...
        assert response.status_code == 200  # El endpoint maneja la excepción y devuelve 200 con success=False
        data = response.json()
        assert data["success"] is False
...
class TestInfoEndpoint:
    ...
    def test_get_info_success(self, client, mock_ui_service):
        ...
        mock_ui_service.get_date_range_available.return_value = (date(2002, 6, 1), date(2024, 12, 1))
...
class TestStartupEvent:
    ...
    def test_startup_event_with_existing_data(self, mock_processor, mock_service_class, mock_session_local):
    ...
    @patch('main.UIService')
    @patch('main.excel_processor')
    def test_startup_event_no_data_success(self, mock_processor, mock_service_class, mock_session_local):
    ...
    @patch('main.UIService')
    @patch('main.excel_processor')
    def test_startup_event_no_data_failure(self, mock_processor, mock_service_class, mock_session_local):
