...
def test_brou_endpoint():
    ...
    try:
        response = requests.get('http://127.0.0.1:8000/api/brou/current?full=true', timeout=10)
...
def test_health_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            brou_checks = [check for check in data.get('checks', []) if check.get('name') == 'brou_cache']
            if brou_checks:
                brou_check = brou_checks[0]
                print(f'BROU cache status: {brou_check.get("status")}')
                print(f'BROU cache message: {brou_check.get("message")}')
                print(f'BROU cache details: {json.dumps(brou_check.get("details"), indent=2)}')
                return True
            else:
