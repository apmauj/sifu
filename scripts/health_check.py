...
class SifuHealthChecker:
    ...
    def check_security(self):
        ...
        if success:
            try:
                ...
                self.results["SEC-CHK-001"] = {
                    "status": "ERROR",
                    "message": "Error parseando pip-audit",
                }
        else:
        ...
        success, stdout, stderr = self.run_command(
            'grep -r "validate\|sanitize" --include="*.py" . | wc -l', shell=True
        )
        if success:
        ...
        if success:
            try:
                ...
                self.results["OBS-CHK-001"] = {
                    ...
                    "message": "Health check con timestamp"
                    if has_timestamp
                    else "Health check sin timestamp",
                }
            except Exception:
                self.results["OBS-CHK-001"] = {
                    "status": "ERROR",
                    "message": "Error parseando health check",
                }
        else:
            self.results["OBS-CHK-001"] = {
                "status": "FAIL",
                "message": "Health check no disponible",
            }
    ...
    def check_performance(self):
        ...
        if success:
            try:
                ...
                self.results["PERF-CHK-002"] = {
                    "status": "ERROR",
                    "message": "Error midiendo latencia",
                }
        else:
            self.results["PERF-CHK-002"] = {
                "status": "FAIL",
        ...
        if success:
            ...
            if success:
                try:
                    ...
                    score = (
                        lighthouse_data.get("categories", {})
                        .get("accessibility", {})
                        .get("score", 0)
                        * 100
                    )
                    ...
                    self.results["UX-CHK-002"] = {
                        "status": "ERROR",
                        "message": "Error parseando Lighthouse",
                    }
            else:
                self.results["UX-CHK-002"] = {
                    "status": "SKIP",
                    "message": "Lighthouse no ejecutado",
                }
        else:
            self.results["UX-CHK-002"] = {
                "status": "SKIP",
                "message": "Lighthouse no instalado",
            }
    ...
    def generate_report(self):
        ...
        if total_checks > 0:
            score = (
                (status_counts["PASS"] * 1.0 + status_counts["WARN"] * 0.5)
                / total_checks
                * 100
            )
