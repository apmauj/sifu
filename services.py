...
class UIService:
    ...
    def get_ui_by_date_range(self, start_date: date, end_date: date) -> List[UIValue]:
        ...
        try:
            records = self.db.query(UIRecord).filter(
                and_(
                    UIRecord.date >= start_date,
                    UIRecord.date <= end_date
                )
    ...
    def get_ui_closest_to_date(self, target_date: date) -> Optional[UIValue]:
        ...
        try:
            ...
            exact_record = self.db.query(UIRecord).filter(UIRecord.date == target_date).first()
            if exact_record:
                return UIValue(date=exact_record.date, value=exact_record.value)
            
            # If not found, search for closest previous date
            closest_record = self.db.query(UIRecord).filter(
                UIRecord.date <= target_date
            ).order_by(UIRecord.date.desc()).first()
...
class URService:
    ...
    def get_ur_by_year_month(self, year: int, month: int) -> Optional[URValue]:
        ...
        try:
            record = self.db.query(URRecord).filter(
                URRecord.year == year,
    ...
    def get_ur_by_year(self, year: int) -> List[URValue]:
        ...
        try:
            records = self.db.query(URRecord).filter(
                URRecord.year == year
            ).order_by(URRecord.month).all()
            
            return [URValue(year=record.year, month=record.month, value=record.value) for record in records]
            
        except Exception as e:
            logger.error(f"Error getting UR by year {year}: {e}")
            return []
    
    def get_ur_by_range(self, start_year: int, start_month: int, end_year: int, end_month: int) -> List[URValue]:
        ...
        try:
            ...
            records = self.db.query(URRecord).filter(
                or_(*conditions)
            ).order_by(URRecord.year, URRecord.month).all()
            
            return [URValue(year=record.year, month=record.month, value=record.value) for record in records]
            
        except Exception as e:
            logger.error(f"Error getting UR by range {start_year}-{start_month} to {end_year}-{end_month}: {e}")
            return []
    
    def get_latest_ur(self) -> Optional[URValue]:
        """Get the most recent UR value"""
        try:
            record = self.db.query(URRecord).order_by(
                URRecord.year.desc(),
                URRecord.month.desc()
            ).first()
...
class ExchangeRateService:
    ...
    def get_exchange_rate_by_date(self, date: date, currency: Optional[str] = None) -> List[ExchangeRateValue]:
        """Get exchange rates for a specific date, optionally filtered by currency"""
        try:
            query = self.db.query(ExchangeRateRecord).filter(ExchangeRateRecord.date == date)
    ...
    def get_exchange_rate_by_currency(self, currency: str, limit: int = 30) -> List[ExchangeRateValue]:
        """Get recent exchange rates for a specific currency"""
        try:
            records = self.db.query(ExchangeRateRecord).filter(
    ...
    def get_exchange_rate_by_date_range(self, start_date: date, end_date: date, currency: Optional[str] = None) -> List[ExchangeRateValue]:
        ...
        try:
            ...
            records = query.order_by(ExchangeRateRecord.date, ExchangeRateRecord.currency).all()
    ...
    def get_latest_exchange_rates(self, currencies: Optional[List[str]] = None) -> List[ExchangeRateValue]:
        """Get the most recent exchange rates for all or specified currencies"""
        try:
            # Get the latest date
            latest_date_query = self.db.query(ExchangeRateRecord.date).order_by(ExchangeRateRecord.date.desc()).first()
            ...
            query = self.db.query(ExchangeRateRecord).filter(ExchangeRateRecord.date == latest_date)
    ...
    def get_exchange_rate_closest_to_date(self, target_date: date, currency: str) -> Optional[ExchangeRateValue]:
        """Get exchange rate closest to a date for a specific currency"""
        try:
            # Search for exact date first
            exact_record = self.db.query(ExchangeRateRecord).filter(
                and_(
            ...
            # If not found, search for closest previous date
            closest_record = self.db.query(ExchangeRateRecord).filter(
                and_(
                    ExchangeRateRecord.date <= target_date,
                    ExchangeRateRecord.currency == currency.upper()
                )
            ).order_by(ExchangeRateRecord.date.desc()).first()
            ...
            logger.error(f"Error getting exchange rate closest to {target_date} for {currency}: {e}")
            return None
    ...
    def get_date_range_available(self) -> tuple[Optional[date], Optional[date]]:
        ...
        try:
            min_date = self.db.query(ExchangeRateRecord.date).order_by(ExchangeRateRecord.date.asc()).first()
            max_date = self.db.query(ExchangeRateRecord.date).order_by(ExchangeRateRecord.date.desc()).first()
